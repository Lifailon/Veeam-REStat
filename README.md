...
Для установки/обновления модуля, скачайте и запустите скрипт **Deploy-Module.ps1**

При первом запуске необходимо заполнить **Credential** для подключения к серверу VBR, которые сохраняются (в директории с модулем) в файл с именем сервера в формате **xml** с применением **System.Management.Automation.PSCredential** для последующего подключения.
