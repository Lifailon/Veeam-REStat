...
При первом запуске необходимо заполнить **Credential** для подключения к серверу VBR, которые сохраняются в файл с именем сервера в формате **xml** для последующего подключения.
